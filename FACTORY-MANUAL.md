...
The packet is also **bound**: before presenting it the driver runs `factory gate --bind`, snapshotting exactly what you're reviewing (the item's artifact files and its PR pointers, content-hashed). Binding decides nothing — the gate still waits on you. If any of it changes before your decision lands, `factory gate --decision` refuses and names what moved — you re-review the changed part instead of approving blind. What you approve is what you saw; `--accept-drift` exists for deciding *with the change in view*.
