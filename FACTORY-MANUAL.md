...
One backstop worth knowing at intake: work whose text touches sensitive ground (auth, payments, secrets, migrations, …) enters at **high risk** automatically, and stations can raise its risk but never lower it back below that floor — so an under-triaged auth change can't slide past a gate on a low rating. Your explicit `--risk` at creation always wins (the bypass is recorded in the item's history).
...
The packet is also **bound**: the driver saves it under the item and runs `factory gate --open --packet <file>`, snapshotting exactly what you're reviewing (the packet, the artifact files, the PR pointer). If any of it changes before your decision lands, `factory gate --decision` refuses and names what moved — you re-review the changed part instead of approving blind. What you approve is what you saw; `--accept-drift` exists for deciding *with the change in view*.
