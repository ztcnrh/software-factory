...
**Scope guarantee.** The installer only ever touches factory-owned paths (the list below). `--force` overwrites *those* — never anything else the project keeps under `.claude/` or elsewhere — and a path it overwrote rather than created stays out of the manifest, so uninstall can't take a directory you already owned. Two shared files get special handling in every mode: `settings.json` is always **merged** (per hook *entry*, so an event you already hook keeps yours and gains ours; uninstall removes only ours), and `CLAUDE.md` is only ever written between its `<!-- factory:begin/end -->` markers.
...
**Uninstalling.** `--uninstall` removes exactly what the installer created (per the manifest), strips the CLAUDE.md, `.gitignore`, and `.gitattributes` blocks, and unmerges the factory's settings entries — your own files are untouched. `.factory/` (work items, metrics) is deliberately left behind; delete it manually for a clean slate. `--dry-run` works here too.
