...
This is how "raise the % shipped without a human" becomes a *consequence* of the learning loop rather than a setting you flip. You don't tell the factory to skip gates; you teach it which work is safe, one signed policy at a time, and the metric follows.
...
3. The spec was revised, approved, implemented (with validation + four regression tests), reviewed, verified (valid → 201, invalid → 422), and shipped. Metrics: auto-ship 0%, and the ledger pointed at `spec_review` as the worst gate.
