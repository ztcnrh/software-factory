...
- **`.claude/skills/factory-*/SKILL.md`** — one skill per station, 1:1 with the states in `line.yml`. A skill is the station's instruction manual, and every one ends the same way: an output contract telling the agent which `factory advance --verdict …` call to make. Read `factory-triage` and `factory-spec` fully to get the pattern; skim the rest. Two non-station helpers sit alongside: the spec station is a thin coordinator that drives **`write-product-spec`** and **`write-tech-spec`** (the skills that own the actual PRODUCT.md / TECH.md shape — separated so the highest-leverage station has room to be exhaustive), and **`council`** convenes a multi-angle panel for contested, high-stakes calls (its cross-critique second round included).
- **`.claude/agents/factory-*.md`** — the vehicles that run the skills: thin subagent wrappers whose real job is pinning a cost-appropriate model per station (opus for spec and code_review, fable for retro, sonnet elsewhere; the deferred monitor is parked under `deferred/` on haiku), **preloading the station's skill** (`skills:` frontmatter — the contract is injected at startup, not discovered), scoping tools (exhaustive `tools:` lists; verify adds the `claude-in-chrome` MCP for browser evidence, spec/implement add web research, spec/code-review add `Agent` to convene councils as nested subagents), and giving each station an isolated context window.
