...
- **`brief.py`** — composes the deterministic half of a station run's context packet (`factory brief` → `runs/<state>-<attempt>-brief.md`): pure templating of state the engine owns, with one marked section for the driver's session-only additions (closed by design for `checking: true` stations). The trace layer for the driver→station handoff.
