...
Each subagent names its model. Push mechanical stations down (the deferred Monitor, parked under `deferred/`, is on haiku), keep the workhorse stations on sonnet, lift the judgment stations where a miss propagates or ships — Spec (planning leverage), Code-review (the correctness/security backstop), and the rarely-run Retro (it rewrites the factory itself) — to opus. If a station underperforms, the cheapest fix is usually a sharper `SKILL.md`, not a bigger model — and the Retro station will often propose exactly that edit.
