...
Each subagent names its model. Push mechanical stations down (the deferred Monitor, parked under `deferred/`, is on haiku), keep the workhorse stations on sonnet, lift the judgment stations where a miss propagates or ships — Spec (planning leverage) and Code-review (the correctness/security backstop) — to opus, and reserve the top-tier `fable` for the rarely-run Retro. If a station underperforms, the cheapest fix is usually a sharper `SKILL.md`, not a bigger model — and the Retro station will often propose exactly that edit.
