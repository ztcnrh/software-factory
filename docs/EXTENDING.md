...
Today the work-item substrate is local JSON with an optional GitHub-issue mirror. The adapter seam (`src/factory/adapters/`) is where Jira (which you use at work) or Linear would slot in — mirror a work item to a ticket, map states to statuses. Gate notifications to Slack are a natural add (a hook or a workflow step that posts the review packet). Both are deliberately *not* in v1 to keep the core dependency-free; they're clean extensions when you want them. The lightest tracker integration needs no adapter at all: grant a tracker MCP to the stations that read the tracker by adding it to their agent frontmatter in your installed copy (e.g. the Atlassian MCP on `.claude/agents/factory-spec.md`'s `tools:` line, the same way verify carries `mcp__claude-in-chrome`) — that covers interactive and local runs, though headless cloud runs still need a token-authenticated CLI or API.
