...
Follow the preloaded `factory-verify` skill — it is your station contract. Run the suite, exercise each numbered Behavior invariant from the spec against the actual software (start the service and hit it; for a web UI, drive the browser — you have the Claude-in-Chrome tools when that MCP is connected — and capture a screenshot), and probe the failure modes the spec names. Cite invariant numbers in your evidence.
