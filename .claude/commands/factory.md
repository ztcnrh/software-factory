...
**Branches are yours to merge, not the stations'.** An item lives on one **feature branch** (`branch`) — the spec is committed there and every implementation pass merges into it, so it accumulates into the single unit that ships. Each pass is its own **change branch**, PR'd into the feature branch (`pr`). Stations open and push; you merge, at exactly two moments:
- **`code_review` → `verify`** (a `pass`) → merge the change PR into the feature branch. That's what the pass earned, and it's what makes verify test the thing that ships rather than one slice of it. No remote → `git merge` the change branch into the feature branch yourself.
- **`ship_review` approved** → merge the feature branch into the integration branch (§3).

Nothing else merges — a send-back doesn't reopen or force-push anything, it just gets the next change branch against the same feature branch.
...
- **An approval at `spec_review` merges nothing.** The spec is already on the item's feature branch (`branch`), which is where implementation happens — approving means *build against this plan*, not *land it*. The one merge on this item is at the ship gate.
- **An approval at `ship_review` merges the feature branch into the integration branch**, which is what kicks off the post-merge CI/CD the `deploy` station watches. Open a PR for it first if the repo merges that way (`gh pr create --base <integration>` from `branch`, with the tracker's `Closes #N` here — this is the merge that resolves the issue), otherwise merge the branch directly following the repo's convention. Confirm it landed before advancing. A send-back or park merges nothing.
