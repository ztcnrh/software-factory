...
2. Dispatch on `type`:
   - **run_station** → run that station, then make the `factory advance <id> --verdict ...` call its skill specifies. *Where* it runs matters:
     ...
     - Producing stations (`triage`, `spec`, `implement`) may apply their skill (`factory-<station>`) directly in this session for low-risk or automatable items; spawn the subagent for anything bigger. Either way, a station works only from persisted artifacts (the work item, `specs/`, the diff) — never from this chat's memory.
