...
2. Dispatch on `type`:
   - **run_station** → run that station. Every station run ends with the `factory advance <id> --verdict ...` call its skill specifies, and **whoever ran the station runs that command**: a spawned subagent runs it itself before reporting back (so by the time you read its report, the board has already moved); if you worked inline, you run it. *Where* the station runs matters:
     ...
     - **Loop-backs may resume instead of respawn.** When a producing station's work bounces back in the same session (e.g. `code_review --changes_requested--> implement`), you may resume the original station subagent (SendMessage) with the persisted worklist instead of spawning fresh — it keeps its build context. Resume once; if the same item bounces again, spawn fresh (stale context is now the likelier problem). Never resume a checking station — fresh eyes are their whole value. Durable state stays the contract either way: everything a next run needs must be in the item's history/notes/artifacts, because a cloud run can't resume anything.
