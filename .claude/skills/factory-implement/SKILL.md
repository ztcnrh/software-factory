...
- If review returns `changes_requested`, read the latest `.factory/work-items/<id>/code-review-<n>.md` (registered in the item's artifacts; fall back to the review `--notes` in the item history if no file exists) — it carries the numbered worklist *and* the reviewer's rationale. Every `🚨 [CRITICAL]`, `⚠️ [IMPORTANT]`, and `💡 [SUGGESTION]` needs an answer; `🧹 [NIT]` items are yours to take or leave. An answer is a fix **or** a reasoned decline — declining is legitimate and sometimes right, but do it explicitly and in writing, because a reviewer reads a silent skip as an oversight and sends the item back again. Don't reopen settled points. Before you advance, append your half to that same file — a `## Response — implement` section stating, per worklist item, what you changed and why, or why you declined — so the re-review reads a conversation, not a mystery diff.
