...
gh pr view <pr> --json title,body,url,headRefOid,reviews,comments
factory diff <n>
factory threads <n>
...
- The PR's conversation `comments` are where a human states a bar that belongs to no single line. Judge the change against those the way you judge it against the spec, and check that the implementer answered each one; an unanswered human comment is `⚠️ [IMPORTANT]` in `body`.
