...
- **The finished items' pull requests**, when the repo has a remote: `factory feedback <id>` on each item that reached `done` or `parked` since the last retro. Review conversations live on the PRs now, and they carry what intervention records structurally can't: the human's asks with `file:line` precision, the back-and-forth that shows *why* a fix loop happened — and the positive signal. An intervention only ever records a steer, so it can only teach you what failed; a thread the human resolved without pushback, an approving comment, a pass with no threads at all — those say what to *keep*, and a retro that only reads failures will eventually optimize away something that was working.
...
- If the briefing reads thinner than the board suggests it should, run `factory doctor` — it flags the two ways your evidence goes quiet without anything failing: a ledger row whose `category` matches no intervention record (the recurrence check joins on that exact string), and an intervention file whose machine block no longer parses (it still counts as a steer but drops out of every category join).
...
3. **A ledger row per proposal** — record each with `factory ledger add` (title, `--lever`, the `--file`s it touches, the `--answers` it cites, the `--signal`, and `--category` when the proposal answers a specific intervention category — that's what powers the briefing's mechanical recurrence check; see **Managing the ledger**). The ledger plus the PR is the durable provenance — which is exactly why nothing goes inline into the artifacts you edit.
...
- **Substantive rework** (re-cluster, re-derive the proposals): send it back to the retro station. If the original retro subagent is still reachable in this session, **resume it** rather than spawning a new one — it keeps the full context of the interventions it mined, which a fresh spawn would re-derive lossily from the briefing. Otherwise re-run the station fresh in clean, isolated context.
