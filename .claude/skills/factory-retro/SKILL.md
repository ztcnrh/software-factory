...
gh api repos/<owner/repo>/issues/<pr>/comments     # the PR's conversation: a human's unanchored notes
gh api repos/<owner/repo>/pulls/<pr>/commits       # a commit whose author is not `factory` is a human fix
```

Start with the metrics: cost per shipped item and where the steers concentrate; each item row carries its `pr` number. For each item in the window, read the gate comments (the `--why` a human gave), human reviews, conversation comments, and replies to factory findings, and human commits on factory PRs. A human's conversation comment on a PR that then merged is the softest steer there is: something the human wanted said but not fixed, which is exactly the pattern a skill edit can pre-empt. Read the finished items' threads too, resolved ones included: an approving comment or a pass with no threads says what to keep, and a retro that only reads failures eventually optimizes away something that worked.
