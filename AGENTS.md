...
GitHub is the state store, GitHub Actions is the runtime, and `claude -p` is the worker. An issue is a work item, its `factory:*` labels are its state, its comments are its log, and its `<type>/<issue>-<slug>` branches and pull requests are its artifacts. Stations are Claude Code skills the reusable workflow runs headlessly on a packet of context it prepared; the skills come from a factory definition, a repository any number of product repositories name from their one caller workflow. Humans decide by applying a ready label, merging a PR, or requesting changes on one. Nothing about an item is stored anywhere else, and nothing runs outside Actions: not adoption, not a station, not a retro.
...
- `skills/factory-*` are the stations, and `skills/` is the factory definition this repo ships: the tree an adopter copies into a definition of their own and names in the caller's `definition:` line. It is not `.claude/skills/` on purpose; no checkout offers a station as a slash command.
- `.github/workflows/factory.yml` is the reusable workflow; `.github/scripts/run-station.sh` is the one `claude -p` invocation, which stages the definition's skills and refuses a product checkout carrying its own; `templates/factory.yml` is the caller a product repository commits, and the only file it holds. `docs/ARCHITECTURE.md` has the labels, events, the definition, jobs, and packets.
...
- The stations run in the product repository's Actions; the definition supplies only the procedure. Skills are never copied into a product repository, and a station is never driven from a laptop.
...
- `make check` before hand-off. A prompt, skill, or workflow change can only be proven by a live run in a throwaway repository with the caller committed; ask before starting one, it costs real tokens.
