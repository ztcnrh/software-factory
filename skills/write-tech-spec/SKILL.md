...
3. **Testing & validation** — how the implementation will be proven against `PRODUCT.md`'s numbered Behavior invariants: each important invariant maps to a concrete test or verification step — unit/integration tests, manual steps, and, for visual or UI changes, screenshots or a short recording. This is the evidence whoever verifies and signs off relies on before shipping, so it owns proving the feature works; `PRODUCT.md` deliberately has no validation section. An invariant that pins an exact value (bit-identity, byte-identical output, "unchanged from today") maps to a fixture chosen to falsify it; a row whose fixture cannot fail certifies nothing. Run those fixtures while writing the map: that is also how two exactness invariants that cannot both hold surface on paper rather than at review. The map lives here, not in the tests: reference invariants by number in this section and never ask a test name or docstring to cite one. The first renumbering turns such a citation stale for anyone holding only the checkout, which is why the implement and review stations treat it as a finding.
