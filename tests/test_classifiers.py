...
Labels are gate-policy inputs (`classifiers_any` / `classifiers_all`), so a wrong label is a
...
from factory.dispatch import ClassifierError, Dispatcher
from factory.model import ClassifierEvent, StationReport, WorkItem
from factory.store import Store


def test_a_persisted_item_round_trips_its_classifier_log(factory_root: Path):
    """`classifiers` is derived, so it must never reach disk — a stored copy is exactly
    ...
    item.add_classifier("bug", by="triage")
    item.add_classifier("security", by="spec")
    store.save(item)

    on_disk = json.loads((store.dir / "WI-0001.json").read_text())
    assert "classifiers" not in on_disk
    assert [e["name"] for e in on_disk["classifier_log"]] == ["bug", "security"]
    assert store.load("WI-0001").classifiers == ["bug", "security"]


def test_an_item_with_no_classifier_log_loads_unclassified(factory_root: Path):
    """A hand-written or minimal item record must load, not raise — the log is
    optional state, and an absent one means "unclassified", not a broken item."""
    item = WorkItem.from_dict({"id": "WI-0002", "title": "unclassified"})
    assert item.classifiers == []
    assert item.classifier_log == []
...
def test_a_retracted_classifier_disappears_from_the_list_but_not_the_log(factory_root: Path):
    """Correction without erasure: `classifiers` reflects only what is currently applied,
    ...
    item = d.new_item("Session leak", classifiers=["bug"], classifiers_by="human:tianchi")
    _advance(d, item, "needs_spec", classifiers=["docs"])
    d.retract(item, "docs", by="human:tianchi", reason="mis-classified")
    assert item.classifiers == ["bug"]
    assert [(e.name, e.action) for e in item.classifier_log] == [
        ("bug", "add"),
        ("docs", "add"),
        ("docs", "retract"),
    ]
...
def test_re_adding_a_retracted_label_makes_it_active_again(factory_root: Path):
    ...
    d = Dispatcher(factory_root)
    item = d.new_item("Item", classifiers=["bug"], classifiers_by="human:t")
    d.retract(item, "bug", by="human:t", reason="wrong")
    _advance(d, item, "needs_spec", classifiers=["bug"])
    assert item.classifiers == ["bug"]
    assert len(item.classifier_log) == 3
...
def test_re_applying_an_active_label_records_nothing_new(factory_root: Path):
    ...
    d = Dispatcher(factory_root)
    item = d.new_item("Item", classifiers=["bug"], classifiers_by="human:t")
    _advance(d, item, "needs_spec", classifiers=["bug", "bug"])
    assert item.classifiers == ["bug"]
    assert len(item.classifier_log) == 1


def test_a_station_cannot_retract_a_human_applied_classifier(factory_root: Path):
    ...
    d = Dispatcher(factory_root)
    item = d.new_item("Item", classifiers=["bug"], classifiers_by="human:tianchi")
    before = (d.store.dir / f"{item.id}.json").read_bytes()
    with pytest.raises(ClassifierError, match="human:tianchi"):
        _advance(d, item, "needs_spec", retractions=[("bug", "I disagree")])
...
def test_a_station_can_retract_what_another_station_applied(factory_root: Path):
    ...
    _advance(d, item, "needs_spec", classifiers=["data-isolation"])
    _advance(
        d,
        item,
        "ready_for_review",
        classifiers=["observability"],
        retractions=[("data-isolation", "reproduced it: state is shared, not leaked across users")],
    )
    assert item.classifiers == ["observability"]
    assert item.classifier_log[-1].reason.startswith("reproduced it")


def test_retracting_an_absent_classifier_is_refused_at_the_boundary(factory_root: Path):
    """A retraction naming a classifier the item never carried is a caller mistake, not
    a no-op — silently succeeding would hide a typo'd classifier forever."""
    d = Dispatcher(factory_root)
    item = d.new_item("Item", classifiers=["bug"], classifiers_by="human:t")
    with pytest.raises(ClassifierError, match="does not carry"):
        _advance(d, item, "needs_spec", retractions=[("bugg", "typo")])
    with pytest.raises(ClassifierError, match="does not carry"):
        d.retract(item, "bugg", by="human:t", reason="typo")
...
def test_a_retraction_requires_a_reason(factory_root: Path):
    ...
    d = Dispatcher(factory_root)
    item = d.new_item("Item", classifiers=["bug"], classifiers_by="human:t")
    with pytest.raises(ClassifierError, match="reason"):
        d.retract(item, "bug", by="human:t", reason="  ")
...
def test_a_human_can_retract_a_label_of_any_provenance(factory_root: Path):
    ...
    d = Dispatcher(factory_root)
    item = d.new_item("Item", classifiers=["bug"], classifiers_by="human:someone-else")
    _advance(d, item, "needs_spec", classifiers=["docs"])
    for name in ("bug", "docs"):
        d.retract(item, name, by="human:tianchi", reason="cleaning up at the gate")
    assert item.classifiers == []
...
def test_an_unrecognized_label_is_recorded_and_marked_never_dropped(factory_root: Path):
    ...
    d = Dispatcher(factory_root)
    item = d.new_item("Item", classifiers=["data-isolation"], classifiers_by="human:t")
    assert item.classifiers == ["data-isolation"]
    assert d.classifiers.unrecognized(item.classifiers) == ["data-isolation"]
    assert d.classifiers.recognized(item.classifiers) == []


def test_an_unrecognized_label_never_satisfies_a_gate_policy(factory_root: Path):
    """The reason recognition matters: an approved rule keyed on `classifiers_any` must
    not fire on a classifier nobody promoted into the vocabulary. Failing closed sends
    the gate back to the human, which is the safe direction."""
    (factory_root / "policies.yml").write_text(
        "version: 1\ndefault: require_human\nrules:\n"
        "  - id: docsy\n    gate: spec_review\n    decision: approved\n"
        "    when: {classifiers_any: [docs, doc-update]}\n    approved_by: tianchi\n"
    )
    (factory_root / "classifiers.yml").write_text("version: 1\nclassifiers:\n  - docs\n")
    d = Dispatcher(factory_root)
    typo = d.new_item("Item", classifiers=["doc-update"], classifiers_by="human:t")
    assert d.policies.auto_decision("spec_review", typo) is None
    real = d.new_item("Item", classifiers=["docs"], classifiers_by="human:t")
...
def test_a_label_event_carries_its_own_timestamp(factory_root: Path):
    ...
    ev = ClassifierEvent(name="bug", action="add", by="triage")
